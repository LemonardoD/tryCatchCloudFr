...
export const load: PageServerLoad = ({ fetch, url, cookies }) => {
	...
	const fetchApi = async () => {
		if (tag && tag !== "all") {
			let reqUrl = `https://trycatchcloud.fly.dev/api/err-log/by-tag?tag=${tag}`;
			if (range !== "For all time") {
				const timeStamp = getTimeStamp(range as DateRange);
				reqUrl = `https://trycatchcloud.fly.dev/api/err-log/by-tag?tag=${tag}&time=${timeStamp}`;
			}
		...
		if (tag === "all" && range !== "For all time") {
			const timeStamp = getTimeStamp(range as DateRange);
			const apiResponse = await fetch(`https://trycatchcloud.fly.dev/api/err-log/by-time/${timeStamp}`, {
				method: "GET",
				mode: "cors",
				headers: {
					"Content-Type": "application/json",
